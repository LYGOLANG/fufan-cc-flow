...
// See endpoint.ts for the rationale.
...
async function request<T>(
  ...
  if (!res.ok) {
    ...
    apiError.code = err.code || err.error?.code;
    apiError.details = err.details || err.error?.details;
    throw apiError;
  }
  return res.json();
}

export type ProjectTemplateItemName = ".claude" | ".codex" | ".agents" | "AGENTS.md";
export type ProjectTemplateItemType = "directory" | "file";
...
export const api = {
  ...
  // ── Projects ──
  previewProjectInit: (targetPath: string) =>
    request<ProjectInitPreview>("/projects/init/preview", {
      method: "POST",
      body: JSON.stringify({ targetPath }),
    }),
  initProject: (targetPath: string, decisions: ProjectInitDecision[]) =>
    request<ProjectInitResult>("/projects/init", {
      method: "POST",
      body: JSON.stringify({ targetPath, decisions }),
    }),
  ...
  // ── MCP ──
  getMcpServers: (project?: string, engine?: string) => {
    const qs = new URLSearchParams();
    if (project) qs.set("project", project);
    if (engine) qs.set("engine", engine);
    const suffix = qs.toString() ? `?${qs.toString()}` : "";
    return request<{ servers: McpServer[]; readonly?: boolean; engine?: string }>(
      `/mcp/servers${suffix}`
    );
  },
  addMcpServer: (opts: {
  ...
  systemApi: {
    ...
    getInterruptedTasks: () =>
      ...
      }>("/system/interrupted-tasks"),
    clearInterruptedTasks: () =>
      request<{ success: boolean }>("/system/interrupted-tasks/clear", { method: "POST" }),
    getUsage: (provider?: "anthropic" | "codex") =>
      request<{
        available: boolean;
    ...
    // ── OpenAI Codex 引擎 ──
    getCodexInfo: () =>
      request<import("../stores/systemStore").CodexInfo>("/system/codex-info"),
    getCodexAuthStatus: () =>
      request<import("../stores/systemStore").CodexAuthStatus>("/system/codex-auth-status"),
    codexLogin: () =>
      request<{ success: boolean; output: string; alreadyLoggedIn?: boolean }>(
        "/system/codex-login",
        { method: "POST" }
      ),
    codexLoginApiKey: (apiKey: string) =>
      request<{ success: boolean; output: string }>("/system/codex-login-apikey", {
        method: "POST",
