...
interface McpState {
  servers: McpServer[];
  loading: boolean;
  /** 当前列表是否只读(codex 引擎的 MCP 由 codex CLI 管理,本应用不增删改)。 */
  readonly: boolean;

  loadServers: (engine?: string, project?: string) => Promise<void>;
  addServer: (opts: {
    name: string;
    transport: string;
    url?: string;
...
export const useMcpStore = create<McpState>((set) => ({
  ...
  loadServers: async (engine, project) => {
    set({ loading: true });
    try {
      const { servers, readonly } = await api.getMcpServers(project, engine);
      set({ servers, readonly: !!readonly, loading: false });
    } catch {
      set({ loading: false });
    }
  },
