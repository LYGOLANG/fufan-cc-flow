...
function assistantSenderName(): string {
  try {
    ...
    const { providerId, engine } = useConfigStore.getState();
    const provider = useProviderStore.getState().providers.find((p) => p.id === providerId);
    if (!provider) {
      if (engine === "codex" || providerId === "openai") return "OpenAI (Codex)";
      if (providerId && providerId !== "anthropic") return providerId;
      return "Claude Code";
    }
...
export const useChatStore = create<ChatState>()(
  ...
  streamingText: "",
  statusText: "",
  totalUsage: { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0 },
  totalCost: 0,
  ...
  stopStreaming: (taskResult) =>
    set((s) => {
      ...
      const cacheReadTokens = taskResult?.cacheReadTokens ?? 0;
      return {
        messages: msgs,
        currentAssistantId: null,
        isStreaming: false,
        ...
        totalUsage: {
          ...
          cacheReadTokens: (s.totalUsage.cacheReadTokens ?? 0) + cacheReadTokens,
        },
      };
    }),
    ...
    set((s) => {
      ...
      for (const m of allMsgs) {
        if (m.taskResult) {
          ...
          histCacheRead += m.taskResult.cacheReadTokens ?? 0;
      ...
      return {
        ...
        totalCost: histCost,
        totalUsage: { inputTokens: histInput, outputTokens: histOutput, cacheReadTokens: histCacheRead },
