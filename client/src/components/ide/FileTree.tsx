...
export default function FileTree({ onFileClickOverride }: { onFileClickOverride?: (path: string) => void } = {}) {
  ...
  const setInsertInput = useUIStore((s) => s.setInsertInput);

  const [contextMenu, setContextMenu] = useState<ContextMenuState | null>(null);
  const [inlineInput, setInlineInput] = useState<InlineInputState | null>(null);

  // 右键「引用到对话」→ 把 @相对路径 追加到聊天输入框
  const handleAddToChat = useCallback((nodePath: string) => {
    setContextMenu(null);
    setInsertInput(makeAtRef(nodePath, projectPath) + " ");
  }, [projectPath, setInsertInput]);
...
function ContextMenu({
  ...
  return (
    ...
    >
      <button className={itemCls} onClick={onAddToChat}>
        <MessageSquarePlus size={12} className="text-purple-bright" /> 引用到对话
      </button>
...
function TreeNode({
  ...
  const loadSubtree = useFileStore((s) => s.loadSubtree);
  const [childrenLoading, setChildrenLoading] = useState(false);
  const projectPath = useUIStore((s) => s.projectPath);

  // 懒加载：展开一个「未加载」目录（children === undefined）时向后端取子树
  useEffect(() => {
    if (isDir && expanded && node.children === undefined && !childrenLoading) {
      setChildrenLoading(true);
      loadSubtree(node.path).finally(() => setChildrenLoading(false));
    }
  ...
  return (
    <div>
      <button
        ...
        onDragStart={(e) => {
          // 拖到聊天输入框 → 插入 @相对路径
          e.dataTransfer.setData("text/plain", makeAtRef(node.path, projectPath) + " ");
      ...
      {isDir && expanded && (
        <div className="border-l border-white/10 ml-5 pl-1 flex flex-col gap-0.5">
          ...
          {childrenLoading && node.children === undefined && (
