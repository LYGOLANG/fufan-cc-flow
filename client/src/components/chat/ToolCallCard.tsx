...
const TOOL_META: Record<string, {
  ...
  ExitPlanMode: { Icon: ClipboardCheck, label: "计划审核", color: "text-violet-info border-violet-info/20 bg-violet-info/5" },
};
...
function getToolSummary(tc: ToolCall): string {
  ...
  switch (tc.toolName) {
    ...
    case "ExitPlanMode": {
      // 取计划首个非空行(去掉 markdown 标题符号)做摘要
      const firstLine = String(inp.plan || "")
        .split("\n")
        .map((l) => l.replace(/^#+\s*/, "").trim())
        .find((l) => l.length > 0);
      return firstLine || "开发计划待审批";
    }
    default: {
...
/** ExitPlanMode:计划正文 Markdown 渲染(审批前后都展示计划本体,不是原始 JSON) */
function PlanView({ tc }: { tc: ToolCall }) {
  const plan = String(tc.toolInput.plan || "");
  if (!plan) return <p className="text-xs text-slate-500">（计划内容为空）</p>;
  return (
    <div className="max-h-[420px] overflow-y-auto rounded-lg border border-violet-info/15 bg-white/[0.02] px-3 py-2">
...
const OP_COLORS: Record<string, { label: string; color: string }> = {
  I: { label: "插入", color: "text-emerald-ok" },
  C: { label: "复制",   color: "text-cyan-400" },
  U: { label: "更新", color: "text-blue-400" },
  R: { label: "替换", color: "text-amber-glow" },
  D: { label: "删除", color: "text-rose-err" },
  M: { label: "移动",   color: "text-violet-info" },
  G: { label: "图片",  color: "text-pink-400" },
};

function InputView({ tc }: { tc: ToolCall }) {
  // ExitPlanMode: render the plan as markdown
  if (tc.toolName === "ExitPlanMode") return <PlanView tc={tc} />;
...
export default function ToolCallCard({ toolCall: tc }: Props) {
  ...
  const autoExpanded =
    (isScreenshot && Boolean(tc.result) && !tc.isError) ||
    (isPlanReview && tc.status === "awaiting_permission");
    ...
    <div className={`tool-card-enter border rounded-lg my-1.5 overflow-hidden ${
      ...
      {isAwaitingPermission && (
        <div className="border-t border-amber-glow/20 px-3 py-2 space-y-1.5">
          ...
          {/* Action buttons — 计划审核卡片用语义化文案 */}
          <div className="flex items-center gap-2">
            {isPlanReview ? (
              <ClipboardCheck size={14} className="text-violet-info flex-shrink-0" />
            ) : (
              <ShieldAlert size={14} className="text-amber-glow flex-shrink-0" />
            )}
            <span className={`text-xs flex-1 ${isPlanReview ? "text-violet-info" : "text-amber-glow"}`}>
              {isPlanReview ? "计划已生成，等待你审批" : "需要确认此操作"}
            </span>
            <button
            ...
            >
              {isPlanReview ? "批准执行" : "允许"}
            </button>
            {!isPlanReview && permData?.hasSuggestions && (
              <button
