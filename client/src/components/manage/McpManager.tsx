...
export default function McpManager() {
  const { servers, loading, readonly, loadServers, addServer, addServerJson, removeServer, importFromDesktop } =
    useMcpStore();
  const projectPath = useUIStore((s) => s.projectPath);
  const { providerId, engine } = useConfigStore();
  ...
  // codex 引擎读 codex 自己的 MCP(只读),否则读 Claude 的。切换供应商时自动重载。
  useEffect(() => {
    loadServers(isCodexProvider ? "codex" : undefined, projectPath || undefined);
  }, [loadServers, isCodexProvider, projectPath]);
  ...
  return (
    <div className="p-3 space-y-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={() => loadServers(isCodexProvider ? "codex" : undefined, projectPath || undefined)}
      ...
      {/* Codex 只读提示 */}
      {readonly && (
        <div className="text-[11px] px-3 py-2 rounded-lg bg-violet-info/5 border border-violet-info/15
                        text-slate-400 flex items-center gap-1.5">
          <Lock size={11} className="text-violet-info flex-shrink-0" />
          <span>
            当前引擎 <span className="text-slate-200">Codex</span> 的 MCP 由 Codex CLI 管理，此处只读展示，
            请到 <code className="text-slate-300">~/.codex/config.toml</code> 修改。
          </span>
        </div>
      )}
          ...
          {form.mode === "form" ? (
            <>
              ...
              {isUrlTransport && (
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] text-slate-500 uppercase tracking-wider">请求头</span>
                    <button
