...
export default function MemoryManager() {
  ...
  if (editingFile) {
    ...
    const typeLabel = editingFile.type === "memory" ? "记忆" : editingFile.type === "claudemd" ? "CLAUDE.md" : "规则";
          ...
          <div className="space-y-0.5">
            ...
            {overLimit && (
              <div className="flex items-start gap-1 text-[10px] text-amber-glow/80">
                ...
                <span>超出 200 行的内容不会自动加载，建议拆分到主题文件</span>
              </div>
            )}
          </div>
        )}
        ...
        {autoMemoryOpen && (
          <div className="mt-2 space-y-1.5 pl-1">
            ...
            {(memoryIndex || topicFiles.length > 0) && (
              ...
              >
                <Trash2 size={10} />
                {clearConfirming ? "确认清空？再次点击" : "清空所有记忆"}
              </button>
            )}
          </div>
        )}
