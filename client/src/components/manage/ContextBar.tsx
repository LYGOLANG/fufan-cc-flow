...
import { useSystemStore, type UsageWindow } from "../../stores/systemStore";
...
export default function ContextBar() {
  ...
  // Load subscription usage on mount and refresh every 5 min (usage changes
  // slowly; infrequent polling avoids upstream rate limits). The server also
  // caches for 60s and serves the last good value on transient failures.
  useEffect(() => {
    loadUsage();
    const t = setInterval(loadUsage, 5 * 60_000);
  ...
  const handleCompact = () => {
    ...
    const { model, effort, apiKey, engine, codexModel, codexEffort } = useConfigStore.getState();
