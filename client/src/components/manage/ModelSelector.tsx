...
import { ChevronDown, Brain, Gauge, Sparkles, Plug, CheckCircle2, Settings, RefreshCw } from "lucide-react";
...
// 与 Codex CLI 的 model_reasoning_effort 五档对齐
...
// 扩展思考预算档位:0 = 自适应(SDK adaptive),其余经 thinking.budgetTokens 注入
...
export default function ModelSelector({ direction = "down" }: { direction?: "up" | "down" }) {
  ...
  const { availableModels, modelsLoading, modelsSource, loadModels } = useSystemStore();
        ...
        <div className={`absolute right-0 w-80 rounded-xl bg-obsidian-800 border border-obsidian-600/50 shadow-2xl shadow-black/40 overflow-hidden z-50 ${
          ...
          <div className="p-3 border-b border-obsidian-700/50">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5 text-[11px] uppercase tracking-wider text-obsidian-300 font-medium">
                模型
                {isOfficialAnthropic && (
                  <span
                    className={`normal-case text-[10px] tracking-normal ${
                      modelsSource === "live" ? "text-emerald-ok" : "text-slate-500"
                    }`}
                  >
                    {modelsSource === "live" ? "· 在线列表" : modelsSource === "fallback" ? "· 离线回退" : ""}
                  </span>
                )}
              </div>
              {isOfficialAnthropic && (
                <button
                  ...
                  title="重新拉取模型列表"
                  className="p-1 rounded-md text-obsidian-300 hover:text-amber-glow hover:bg-obsidian-700/60 transition-colors disabled:opacity-50"
                >
            ...
            <div className="space-y-1 max-h-48 overflow-y-auto">
              {options.map((opt) => (
                <button
                  ...
                  className={`w-full flex items-center justify-between gap-2 text-left px-3 py-1.5 rounded-lg text-sm transition-colors ${
                    model === opt.id
                      ? "bg-amber-glow/10 text-amber-glow border border-amber-glow/20"
                      : "text-obsidian-100 hover:bg-obsidian-700/60"
                  }`}
                >
                  ...
                  <span className="text-[10px] font-mono text-slate-500 flex-shrink-0">
                    {formatTokens(opt.contextWindow ?? inferContextMax(opt.id))}
                  </span>
                </button>
              ))}
              {options.length === 0 && (
            ...
            <div className="p-3">
              ...
              {thinking && (
                <div className="mt-2 px-3">
                  <div className="text-[10px] text-slate-500 mb-1.5">思考预算</div>
