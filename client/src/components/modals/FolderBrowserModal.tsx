...
import {
  ...
  Check, X, Loader2, Home, RefreshCw, FolderPlus, Clock,
} from "lucide-react";
...
export default function FolderBrowserModal() {
  ...
  // Project dirs discovered from Claude Code session history (port-independent).
  ...
  useEffect(() => {
    if (folderBrowserOpen) {
      ...
      // Discover project dirs from session history (valid paths only, deduped).
      api.getSessions()
        .then((r) => {
          const paths = (r.sessions as { projectPath?: string; projectPathValid?: boolean }[])
            .filter((s) => s.projectPathValid !== false && !!s.projectPath)
            .map((s) => s.projectPath as string);
          setSessionProjects([...new Set(paths)]);
        })
        .catch(() => setSessionProjects([]));
    }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ── Recent projects ── */}
        {recentList.length > 0 && (
          <div className="px-4 pt-2.5 pb-1.5 border-b border-white/5 flex-shrink-0">
            <div className="flex items-center gap-1.5 mb-1.5">
              <Clock size={11} className="text-slate-500" />
              <span className="text-[11px] text-slate-500 font-medium uppercase tracking-wider">最近项目</span>
            </div>
              ...
              {recentList.map((p) => (
                ...
                >
                  <button
                    ...
                    title={p}
                  >
                    <Folder size={13} className="flex-shrink-0 text-amber-glow/70" />
                    <span className="text-xs text-slate-200 truncate">{baseName(p)}</span>
                    <span className="text-[10px] text-slate-600 truncate hidden sm:inline">{p}</span>
                  </button>
                  <button
                    onClick={() => navigate(p)}
                    title="浏览此目录"
                    className="p-1 rounded text-slate-600 hover:text-amber-glow opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"
                  >
                    <FolderOpen size={12} />
                  </button>
                  <button
                    ...
                    title="从最近项目移除"
                    className="p-1 mr-1 rounded text-slate-600 hover:text-rose-err opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"
                  >
                    <X size={12} />
                  </button>
                </div>
