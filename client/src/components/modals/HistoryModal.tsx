...
export default function HistoryModal() {
  ...
  const { currentSessionId, setSessionId, loadHistoryMessages } = useChatStore();
  ...
  const handleSelectSession = async (session: Session) => {
    ...
    // Clear current messages (aborting any still-streaming task first) and set session ID
    startNewSession();
    ...
    >
      <div className="modal-content glass-panel rounded-2xl w-full max-w-lg flex flex-col shadow-2xl shadow-black/50 max-h-[80vh]">
        ...
        <div className="px-4 py-3 border-b border-white/5 flex-shrink-0 space-y-2">
          <button
            onClick={() => { startNewSession(); setSessionId(""); setHistoryModalOpen(false); if (!projectPath) setFolderBrowserOpen(true); }}
