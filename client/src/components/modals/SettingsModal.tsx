...
import { MODEL_LABELS, type ModelId, type EffortChoice } from "../../types/claude";

// 离线兜底：与后端 FALLBACK_MODELS 对齐的 CLI 别名（联网时由 /system/models 动态覆盖）。
const FALLBACK_MODELS: { id: ModelId; label: string; desc: string }[] = [
  { id: "opus",    label: MODEL_LABELS.opus,    desc: "最强大，适合复杂任务" },
  { id: "sonnet",  label: MODEL_LABELS.sonnet,  desc: "速度与能力的平衡" },
  { id: "haiku",   label: MODEL_LABELS.haiku,   desc: "轻量快速，适合简单任务" },
];

// 推理力度六档，与聊天框 ModelSelector 完全一致（含第六档 ultracode）。
const EFFORT_LEVELS: { id: EffortChoice; label: string }[] = [
  { id: "low",       label: "低" },
  { id: "medium",    label: "中" },
  { id: "high",      label: "高" },
  { id: "xhigh",     label: "超高" },
  { id: "max",       label: "最大" },
  { id: "ultracode", label: "Ultra" },
];
...
export default function SettingsModal() {
  ...
  // 与聊天框一致：优先用 /system/models 的 live 动态列表，离线再退回别名兜底。
  useEffect(() => {
    if (settingsModalOpen && availableModels.length === 0) loadModels();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [settingsModalOpen]);
  const models = availableModels.length > 0
    ? availableModels.map((m) => ({ id: m.id, label: m.label, desc: "" }))
    : FALLBACK_MODELS;
