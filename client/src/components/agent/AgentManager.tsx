...
export default function AgentManager() {
  ...
  const providerModelIds = currentProvider?.models?.length
    ? currentProvider.models
    : isCodexProvider
      ? [model].filter(Boolean)
      : ["opus", "sonnet", "haiku"];
  const modelOptions = Array.from(new Set(
    formModel && formModel !== "inherit" && !providerModelIds.includes(formModel)
      ? [formModel, ...providerModelIds]
      : providerModelIds
  ));
  const formatModelLabel = (id: string) => (MODEL_LABELS[id] || id).replace(/^Claude\s+/, "");
      ...
      <div className="p-3 space-y-3 overflow-y-auto">
        ...
        <div className="flex gap-2">
          <select value={formModel} onChange={(e) => setFormModel(e.target.value)}
            ...
            <option value="inherit">继承当前供应商</option>
            {modelOptions.map((id) => (
              <option key={id} value={id}>{formatModelLabel(id)}</option>
            ))}
          </select>
        ...
        <div className="grid grid-cols-2 gap-2">
          <TagInput label="技能" value={formSkills} onChange={setFormSkills}
            placeholder="输入后回车" />
          <TagInput label="MCP 服务器" value={formMcpServers} onChange={setFormMcpServers}
            placeholder="输入后回车" />
        </div>
  ...
  /* ── Codex 引擎:没有 Claude 式子 Agent 概念,不展示列表 ── */
  if (isCodexProvider) {
    return (
      <div className="p-3">
        <div className="flex flex-col items-center justify-center py-12 gap-3 text-center">
          ...
          <p className="text-xs text-slate-300 font-medium">当前引擎 Codex 不支持子 Agent</p>
          <p className="text-[11px] text-slate-500 max-w-[240px] leading-relaxed">
            子 Agent 是 Claude Code 的能力。Codex 的近似机制是 skills / plugins，
            由 Codex CLI 自行管理。切回 Claude 供应商即可管理子 Agent。
          </p>
        </div>
      </div>
    );
