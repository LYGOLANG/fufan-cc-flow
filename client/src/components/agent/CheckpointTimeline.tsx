...
export default function CheckpointTimeline() {
  ...
  const actorName = currentProvider
            ...
            {isSelected && selected && (
              <div className="mt-1.5 p-3 rounded-xl border border-white/8 bg-white/[0.02] space-y-2.5">
                ...
                {selectedOps.length > 0 && (
                  <OperationsBlock operations={selectedOps} actorName={actorName} />
                )}
                  ...
                  {rollbackState === "idle" && (
                    <div className="space-y-1">
                      ...
                      <RollbackOption
                        ...
                        desc={
                          cp.hasFileChanges
                            ? `恢复 ${cp.changedFiles.length} 个文件到此检查点之前`
                            : "尝试通过 SDK 恢复到此检查点之前"
                        }
  ...
  return (
    <div className="p-3 rounded-xl border border-amber-glow/20 bg-amber-glow/5 space-y-2.5">
      ...
      {hasFiles ? (
        ...
        <p className="text-xs text-slate-400">
          此检查点无文件变更记录，将尝试通过 SDK 恢复到此检查点之前的状态。
...
function OperationsBlock({
  ...
  return (
    <div>
      ...
      >
        {expanded ? <ChevronDown size={9} /> : <ChevronRight size={9} />}
        {actorName} 操作 ({operations.length})
      </button>
      <div className="space-y-1">
        {shown.map((op, i) => (
          <div
