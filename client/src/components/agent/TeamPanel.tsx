  ...
  if (!enabled) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-3 px-6 py-10 text-center">
        ...
        <div>
          <div className="text-sm font-medium text-slate-300 mb-1">Agent 团队未启用</div>
          <div className="text-xs text-slate-500 leading-relaxed mb-3">
            Agent 团队是实验性功能，需要设置环境变量后重启服务。
          </div>
        </div>
        <div
        ...
        >
          ...
          <div className="space-y-1.5 text-xs text-slate-400 font-mono">
            ...
            <p className="text-slate-500 mt-2"># 或在设置页面中添加到 Claude 环境变量</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0">
      ...
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-white/5 flex-shrink-0">
        <span className="text-xs font-medium text-slate-400">Agent 团队</span>
        <button
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-12 gap-3 text-center px-6">
            ...
            <div>
              <div className="text-sm font-medium text-slate-300 mb-1">尚未创建团队</div>
              <div className="text-xs text-slate-500 leading-relaxed">
                Agent 团队允许多个 Agent 通过共享任务列表<br />和邮箱系统自主协作完成复杂任务。
              </div>
            </div>
            <button
              onClick={() => setShowCreator(true)}
