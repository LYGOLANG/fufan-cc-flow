...
import { wsTerminalUrl } from "../../services/endpoint";
...
export default function XTerminal({ termId, cwd }: XTerminalProps) {
  ...
  useEffect(() => {
    ...
    // xterm.js has no built-in `copyOnSelect` constructor option — implement it
    // manually: copy the selection to the clipboard whenever it changes.
    // Disposed automatically by term.dispose() in the cleanup below.
    term.onSelectionChange(() => {
      const sel = term.getSelection();
      if (sel) {
        navigator.clipboard.writeText(sel).catch(() => { /* clipboard permission denied — ignore */ });
      }
    });
    ...
    const ws     = new WebSocket(wsTerminalUrl(params.toString()));
