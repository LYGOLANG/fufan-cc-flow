...
## [v1.16] - 2026-09-03
### 上下文
- **上下文将满时不再自动压缩，改为交接到新会话**（REQUIREMENTS F1.5 已同步）。默认阈值从 95% 降到 90%：达阈值时先让当前会话写一份给接班人的交接文档（任务、已完成及证据、下一步、关键文件、死路、用户偏好、未决问题），拿到后自动开一个真正的新会话（不 resume、不复用常驻进程），把文档当开场白发进去接着干。
- 换掉压缩的理由是实测的：一次 1M 会话被压到 69K，四次累计丢弃 388 万 tokens，用户的直观感受是「换了模型之后突然变傻」——它看到的第一眼就是一份摘要。摘要由模型自己写、写成什么样不受控、也从不给用户看；交接文档是可见可审的一条消息，而且新会话的上下文是真的空的。
- 附带好处：Codex 引擎没有 `/compact`，此前自动压缩对它整个是空操作，交接两条引擎都能用。
- 阈值降到 90% 是因为「写交接文档」那一轮本身也要占上下文，95% 起步容易在写文档时把窗口撑爆。
- 失败一律保守：拿不到文档（模型没照做、用户中止、报错）就保留原会话并在状态栏说明，不自动重试，用户可点「立即交接到新会话」手动来一次。
- 手动压缩按钮保留（上下文栏「压缩当前会话（不新建）」），只在用户显式点击时执行。

---

## [v1.15] - 2026-09-03
### 项目管理
- `+` 添加项目改为**只打开、不写文件**。Agent 模板（`.claude`、`.codex`、`.agents`、`AGENTS.md`）的复制拆成独立动作：项目标签右键「初始化 Agent 模板…」，且不论有无冲突都先弹预览、用户确认才写。此前 `+` 会自动复制模板，空目录时连弹窗都没有；用户三次反馈「去掉自动加 agent」（REQUIREMENTS F4.1 已同步）。
- 内置插件改为只进插件市场、由用户自己点安装（commit e68bb18）。同一原则：改用户环境的动作一律由用户触发。

---

## [v1.14] - 2026-07-27
### 工作流
- 「工作流」升级为具备真实编排能力的功能模块：按步骤顺序执行、步骤间数据传递、单步失败可重试/跳过/中止、运行中可停止。此前「执行」只是把步骤拼成提示词填进输入框，不调度、不等待、不传数据。
- 第一版只做线性编排，明确不做并行分支、条件分支和循环——现有工作流全部是线性的，DAG 需配套图形编辑器且当前无使用场景。
- 一步是否完成以该轮任务真正结束（`task_complete`）为准，不以模型自称完成为准；引擎不判断语义正确性，只判定是否正常结束与输出是否为空。
- 失败时暂停并询问，不静默继续，避免错误顺着数据传递链条扩散。
- 数据结构新增字段一律可选，旧工作流无需重建即可运行。

### 架构
- 记录编排引擎与纯 Rust 迁移的冲突及决策（见 Product-Spec 第 11 节）：第一版在 Node 侧实现，但核心业务逻辑必须与传输层解耦，经「执行单步」抽象接口访问会话能力，使后续迁移是翻译而非重新设计。
- Workflows 的迁移目标从「现有 CRUD」扩展为「CRUD + 编排引擎」；其「行为与迁移前一致」的验收基准改为编排引擎上线后的行为。

### 修正（本轮先行落地）
- 工作流列表按名称排序：此前直接返回 `fs.readdir` 的目录项顺序，而文件名是 `wf_<时间戳>.json`，与用户可见名称无关，导致编号 ①②③ 显示成乱序。
- 「工作流」标签页顶部的运行中区块改称「后台任务运行中」：它显示的是后台任务，与工作流列表无关，原文案让人误以为某个工作流在跑。
- 移除空状态的假承诺文案「创建工作流来自动按步骤调用多个 Agent」，改为如实描述当前行为；执行按钮标注「不会自动发送」。
- 后台任务「已完成」与审计日志改为按时间倒序（最近的在最上面）。
