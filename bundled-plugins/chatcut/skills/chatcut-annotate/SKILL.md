...
description: 当用户要在视频画面上做标注、圈一下某个按钮、加箭头或高亮框、把某处放大强调、给操作步骤加说明卡片时使用。把标注定义编译成 HyperFrames 透明层并合成到画面上。也包括：把定好的十几处标注整批渲成透明层叠到画面上；只动了几处时的增量重渲，命中缓存的别全量重跑；上次出片发现某处高亮框位置偏了要改完重渲；用户说「面板上点了生成标注层没回音、界面卡在进行中」时先认领界面待办（request_claim）再动手；以及 feature:annotation 被 blocked、hyperframes 没装时讲清要下多大、从哪来、装到哪并补齐依赖（dependency_fetch）。
---
