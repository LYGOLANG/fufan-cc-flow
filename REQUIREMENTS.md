...
- **上下文将满时交接到新会话，不压缩**（默认 90%，上下文栏可调，拖到 100% 关闭）：
  - 达阈值时先让**当前会话自己**写一份交接文档（当前任务、已完成及证据、下一步、关键文件、死路与坑、用户偏好、未决问题），要求纯文本输出，不许调工具、不许写文件
  - 拿到文档后自动开一个全新会话（不 resume、不复用常驻进程），把文档作为开场白发进去，让新会话接着干
  - 消息流中插入「已交接到新会话」分隔线，交接文档全文即其下第一条消息，可见可审
  - 只在一轮对话结束时判定：打开一个本来就很满的旧会话不会被误交接
  - 拿不到文档（模型没照做 / 用户中止 / 报错）时保留原会话并在状态栏说明，不自动重试
  - 上下文栏提供「立即交接到新会话」手动触发，与自动交接同一份实现
- **手动压缩仍保留**：上下文栏「压缩当前会话（不新建）」按钮，只在用户显式点击时执行；自动压缩已移除
...
#### F4.1 添加项目与 Agent 模板初始化

顶部 Project Picker 只保留一个 **添加项目** 入口：用户点击 `+` 后选择目标目录，系统**只打开该项目，不向目录写入任何文件**。把 Fufan-CC Flow 的项目规则模板（Agent / Hook / Skill 编排）复制进项目是另一个**显式动作**：在项目标签上右键 →「初始化 Agent 模板…」，写入前必须先展示预览、由用户点确认。

> 2026-09-03 变更：此前 `+` 会自动复制模板，无冲突时连弹窗都没有。用户三次反馈「去掉自动加 agent」——「去掉」指的是不自动，不是「先问我」。故拆成两个动作。

**入口与流程**

1. 用户在顶部 Project Picker 点击 `+` **添加项目**，选择目标目录，系统直接切换到该项目（刷新文件树、终端工作目录和会话上下文）。目录内容不变。
2. 用户在项目标签上右键，选择 **初始化 Agent 模板…**。
3. 系统从当前 `fufan-cc-flow` 根目录读取模板项，检测目标目录中是否已有同名文件或目录。
4. **无论有无冲突，都弹出预览弹窗**，逐项列出将复制 / 冲突 / 源缺失状态；冲突项由用户选择覆盖或跳过。
5. 用户确认后，系统复制被选中的模板项。
6. 写入成功后 Toast 提示，当前项目切到目标目录。
...
「工作流」标签页当前**不具备编排能力**，这一点必须先说清楚，因为界面暗示的能力和实际实现严重不符：

- 点「执行」只调用 `composeAndSend`，把各步骤拼成一段中文提示词，通过 `setPrefillInput` **填进输入框**，还需用户自己回车发送。
- 不调度 Agent、不等待某一步完成、不在步骤间传递数据、不处理失败。
- 步骤中配置的 `agent` 只是变成提示词里的一句「（某某 Agent）：做什么」，最终照不照做取决于主对话模型的即兴发挥。
- 界面却把步骤画成方框、用箭头串联，空状态曾写「创建工作流来自动按步骤调用多个 Agent」——该文案已在本轮先行修正为「存成模板，一键填入输入框」，以消除误导。

本 Phase 的目标：把它变成**由代码控制流程**的真实编排引擎——顺序执行、步骤间数据传递、失败可处置。

#### F5.2 编排模型（第一版范围）

第一版只做**线性编排**，不做并行与条件分支。依据：现有工作流（含「量化总流程（卖方→买方→交易员→个股研报）」）全部是线性的，并行/DAG 需要配套图形编辑器，工期成倍且当前无使用场景。

| 能力 | 第一版 | 说明 |
|------|--------|------|
| 顺序执行 | ✅ | 第 N 步真正结束后才启动第 N+1 步 |
| 步骤间数据传递 | ✅ | 上一步输出可作为变量注入下一步提示词 |
| 失败处置 | ✅ | 单步失败可重试 / 跳过 / 中止整个工作流 |
| 中断 | ✅ | 运行中可随时停止，已完成步骤的产出保留 |
| 并行分支 | ❌ | 留待后续，需 DAG 执行器 + 编辑器重做 |
| 条件分支 | ❌ | 同上 |
| 循环 | ❌ | 同上 |

#### F5.3 步骤执行与数据传递

- 每一步在**同一个会话**内顺序推进，保持上下文连续（与用户手动逐条发送的语义一致）。
- **步骤指定的 Agent 目前是「提示词点名」而非强制分派**：执行时把该步提示词包装成「请使用 X 这个 Agent 完成：…」交给主会话，由模型自行决定是否通过 Task 工具分派。这意味着模型可以无视它、自己把活干了；填一个不存在的 Agent 名也不会报错。
  实测确认：一个指定了不存在 Agent 的步骤照常「成功」。
  真正的强制分派需要在 SDK 的 `agents`（AgentDefinition）层面声明并约束调用，属后续改进；在那之前，界面与文档都不应把它描述成「分派」。
- 每步支持声明一个**输出变量名**；该步结束后，其最终文本产出以该变量名登记，后续步骤可用 `$变量名` 引用（与现有 `variables` 的 `$name` 替换语法一致，用户无需学新写法）。
- 未声明输出变量的步骤不产生变量，仅靠会话上下文延续——保持简单场景零配置。

#### F5.4 完成判定与失败处置（AI 概率性四问）

编排调度的是 AI，每一步的产出是概率性的，必须显式定义：

| 维度 | 规则 |
|------|------|
| **何为一步完成** | 以该步对应的一轮任务真正结束（收到 `task_complete`）为准，而非模型说了「完成」。这是判定能被代码信任的唯一硬信号。 |
| **触发方式** | 仅手动触发（点击执行）。不做定时、不做文件变更自动触发。 |
| **不确定行为** | 模型可能答非所问、输出为空、或未按要求产出可供下一步使用的内容。引擎不做语义正确性判断——那本身又是一次不可靠的 AI 调用；只判定「是否正常结束」与「输出是否为空」。 |
| **降级与兜底** | 单步失败（进程错误 / 被中止 / 输出为空）时暂停整个工作流并提示，由用户选择重试该步、跳过该步、或中止；不静默继续，避免错误顺着数据传递链条扩散。达到费用上限（`maxBudget`）时按现有机制中止并提示。 |

#### F5.5 运行状态与可观测性

- 工作流运行期间，「工作流」标签页显示当前执行到第几步、各步状态（等待 / 运行中 / 成功 / 失败 / 跳过）与耗时。
- 每步的产出可展开查看；失败步骤显示错误原因。
- 与现有能力衔接：步骤内派生的子 Agent 仍在「Sub-Agent 执行树」呈现，生命周期事件仍进「审计」。
- 运行中的工作流与「后台任务」是两类不同对象，界面上不得混为一谈（该误导已在本轮先行修正）。
...
```ts
// 现状
interface WorkflowStep { agent: string | null; prompt: string; }
interface Workflow { id: string; name: string; steps: WorkflowStep[]; variables: string[]; }
```

需扩展步骤级字段（输出变量名、失败策略等）。约束：

- **不得要求用户重建已有工作流**。新增字段一律可选，缺省时行为等价于「无输出变量、失败即暂停询问」。
- 旧数据文件（`.json`）保持可直接读取，读取时按缺省值补全，无需一次性迁移脚本。
- 工作流列表排序按名称（本轮已先行修正，此前是 `fs.readdir` 的目录项顺序，与用户可见的名称无关）。

#### F5.7 实现位置与迁移关系

编排引擎先在 Node 侧实现，但**核心逻辑必须与传输层解耦**：状态机、数据传递、失败策略属纯业务逻辑，通过一个「执行单步」的抽象接口调用底层会话能力。

这样在纯 Rust 迁移（见 `Product-Spec.md`）时，该模块是**翻译**而非重新设计。详见 `Product-Spec.md` 第 8 节记录的冲突与决策。

#### F5.8 验收标准

- Given 一个三步线性工作流，When 点击执行，Then 三步依次执行，第 N+1 步在第 N 步收到 `task_complete` 之后才启动。
- Given 第 1 步声明了输出变量且第 2 步引用了它，When 工作流执行，Then 第 2 步的提示词中该变量已被替换为第 1 步的实际产出。
- Given 某步执行失败，When 失败发生，Then 工作流暂停并提示，用户可选择重试 / 跳过 / 中止，且不会自动继续后续步骤。
- Given 工作流运行中，When 用户点击停止，Then 当前步骤被中止、后续步骤不再启动、已完成步骤的产出保留可见。
- Given 一个在本功能上线前创建的旧工作流，When 用户打开并执行它，Then 无需任何修改即可运行，行为等价于「无输出变量、失败即询问」。
...
| WebView CSP | 桌面壳基线 CSP（`default-src 'self'`）：脚本仅自身、连接仅本地后端 http/ws、图片放行 data/blob/https 外链、object 禁用。即使将来引入 HTML 渲染面，注入脚本也无法外联 |
...
以下需求已提出并完成初步调研，尚未进入开发。记录在此避免重复调研。

### 8.1 远程会话与会话共享（2026-07-27 提出）

两个需求在底层是同一件事：**把一个会话的事件流与输入通道，安全地开放给不在本机的另一方**。因此不做两套，做一套「远程会话通道」，远程服务器与会话共享都是它的应用。

**参考项目澄清**：用户提供的参考 [jimhy/lumen](https://github.com/jimhy/lumen) **不是 SSH 工具**。它是 Rust 写的 GPU 终端，远程能力走自托管架构：用户自行部署 `lumen-server`（Axum），两台设备登录同一账户、输入 9 位配对码建连，能直连走 QUIC P2P、否则走中继。且它明确**不支持**多用户共享同一会话，只支持同账户的多设备远程控制。真正可参考的是其「配对码 + 自建服务 + P2P/中继」的连接模型。

**核心设计（草案）**

- CLI 只在 Host 端运行。Guest 不装 Claude Code、不同步文件、不需要 API Key，本质是远程终端。代价是 Host 必须在线，换来 Guest 零配置。
- 「加密会话 ID」设计为**会话票据**：加密封装 `{连接地址, sessionId, 一次性凭据, 过期时间, 权限档位}`。其中的凭据复用现有接口鉴权令牌机制（见 `server/src/middleware/auth.ts`），不新造轮子。
- 票据必须可过期、可吊销；Host 能查看当前连接方并踢出。遵循「可建必可删」，不做只能加不能撤的共享。

**决策记录**

| # | 问题 | 结论 | 状态 |
|---|------|------|------|
| 3 | 通道走哪条 | 第一版走 **SSH 隧道**：复用现有 web 模式（`endpoint.ts` 的 `isWebOrigin()` 分支即为此设计），零新增服务、无运营成本、数据不过第三方。验证有真实使用后再考虑自建中继。 | **2026-07-28 已拍板** |
| 1 | Guest 触发的工具调用，HIL 权限卡弹给谁 | 建议**只弹给 Host**。工具在 Host 机器上执行、动的是 Host 的文件，确认权就该在 Host。代价是 Host 离开时 Guest 会卡住，但反过来等于「共享一次会话＝交出一台机器」。可选增强：Host 显式授权某 Guest 也能确认。 | 待拍板（共享会话阶段才用到） |
| 2 | 两端同时发消息如何处理（会话串行，一轮未完不能插第二轮） | 建议**谁先到谁执行，运行中另一方输入进队列**并显示「排队中」，不并发。备选：显式发言权交接。 | 待拍板（共享会话阶段才用到） |

**由通道决策推出的实施顺序**

选定 SSH 隧道后，两个需求的底座合一：**前端连到一个不在本机的后端**。

- 远程服务器 = 前端 + 远程机器上由我方拉起、独占的后端
- 会话共享 = 前端 + 他人机器上已在运行、对外开放的后端

差别只在共享多一层协作（消息广播、发言权、权限归属）。故先做「连远程后端」的底座，
共享会话在其上叠加协作层。底座不通，共享无从谈起。决策点 1、2 因此推迟到共享阶段再定。

**与现有 Spec 的关系（2026-07-28 修正）**

此前记为「与 Spec 冲突」，**是调研疏漏**。`Product-Spec.md` 自己就预留了这条路：

- 第 41 行架构决策表：「远程 Web｜后续可选 Axum 适配器｜只复用 Rust service，
  不进入桌面安装包、**不阻塞桌面迁移**」
- 第 57 行 P1 范围：「用 Axum 暴露同一 Rust service，恢复独立浏览器/**SSH 隧道**部署形态」

真实关系不是冲突，而是**排序**：Spec 把 SSH 隧道放在 P1，即 Node→Rust 迁移（P0）完成之后。
而该迁移至今未启动，当前仍是 Node sidecar。

**为什么可以先做**：SSH 隧道方案下后端几乎无需改动——隧道由客户端 `ssh -L` 建立，
后端只管监听自己的端口，不感知对端在哪。工作量集中在「前端 transport 可指向非本机地址」
与「Rust 壳管理隧道生命周期」，这两块在将来迁移到 Rust 后大部分可保留。

**必须遵守的纪律**（Spec 第 71 行，也是编排引擎「零传输层依赖」的同一条纪律）：
前端业务调用必须走统一 transport，组件与 store 不直接依赖 `fetch`/`WebSocket`/`invoke`。
远程能力只在 transport 层实现，业务层不得出现「本机还是远程」的分支——否则将来迁移时
这些分支会散落在几十个组件里，变成翻译不动、只能重写的死账。

**仍需保留的安全姿态**：桌面默认不对外监听（维持现状），仅在用户显式开启远程/共享时
才放开，关闭即停。「不监听端口」从绝对约束调整为默认状态。

**优先级**：工作流编排引擎（Phase 12-14）已于 v0.1.23 完成并发布，此项解除阻塞，
为当前待开发的下一个功能。

---

### 8.2 第一版实现方案（2026-07-28 定稿）

#### 核心思路：远程后端由本机通过 SSH 启动，因而"远程"在前端看来仍是本机

```
本机                                          远程服务器
┌──────────────────────────┐                 ┌────────────────────────┐
│ WebView (React)          │                 │ Node sidecar           │
│   fetch localhost:9999 ──┼── ssh -L 隧道 ──┼─→ 127.0.0.1:3001       │
│                          │                 │   ↓                    │
│ Rust 壳                  │                 │ Claude Code CLI        │
│   ① ssh 启动远程后端      │                 │ 项目文件               │
│      并注入本机生成的令牌  │                 └────────────────────────┘
│   ② 建立端口转发          │
└──────────────────────────┘
...
| 约束 | 位置 | 为何不再是问题 |
|------|------|--------------|
| CSP 只允许 `connect-src localhost/127.0.0.1` | `tauri.conf.json:24` | 隧道映射到本机端口，前端连的仍是 `localhost:<本地端口>` |
| CORS 白名单只放行本地来源 | `server/src/app.ts:17` | Guest 端 Origin 仍是 `tauri://localhost`，在白名单内 |
| 后端只绑 `127.0.0.1` | `server/src/index.ts` | 隧道出口在远程本机，源 IP 即 `127.0.0.1` |

**令牌**：远程后端由我方 SSH 启动，故可像本机一样用 `CC_FLOW_AUTH_TOKEN` 环境变量注入
本机生成的令牌。`main.tsx:60` 那个"渲染前阻塞取令牌"的时序**无需改动**，
也就不存在异步取令牌带来的 401 竞态。

**令牌传递方式**：必须经 **SSH 的 stdin** 送达，不得写进命令行参数。

`sidecar.rs:24` 已确立本机侧的原则——"不进命令行参数（命令行在进程列表里对其它
进程可见）"。远程侧这条只会更严重：远程机器往往是多用户的，`ps aux` 谁都能看，
一旦令牌出现在命令行里，那台机器上的任何用户都能拿它调用完整接口
（读写文件、执行命令、读取存有 API Key 的配置）。

具体形态（一条 ssh 同时建隧道与启动后端，进程退出则两者一并回收）：

```
ssh -L <本地端口>:127.0.0.1:<远程端口> <user>@<host> \
    'read -r CC_FLOW_AUTH_TOKEN; export CC_FLOW_AUTH_TOKEN; exec node <dir>/dist/index.js'
```

令牌随后写入该 ssh 进程的 stdin。不选 `SetEnv` 是因为它要求远程 sshd 配了
`AcceptEnv`，静默失败时会退化成"后端没有令牌 → 整体放行鉴权"，
即安全措施失效却毫无报错——这类失败模式必须避免。

**代价**：远程机器必须能被 SSH 登录，且已安装 Node 与 Claude Code CLI（并完成登录）。
第一版由用户自行部署，提供一键脚本；不做客户端自动推送（先把隧道链路跑通，
否则调试时分不清是传文件的问题还是建隧道的问题）。

#### 与 Phase 11 的关系

`DEV-PLAN.md` 的 Phase 11（P1）是**Rust 迁移完成后**用 Axum 暴露服务的远程形态。
本方案是**Node 阶段的提前实现**，两者并存不矛盾：

- 本方案不新增服务端组件（Node 后端本身就是 HTTP 服务器，隧道直接转发）
- 迁移完成后，Rust 侧只需保留"SSH 启动远程进程 + 建隧道"这段逻辑，
  远程跑的换成 Rust binary 即可，前端与隧道管理代码可直接沿用

#### 必须遵守的实现纪律

1. **业务层不得出现"本机还是远程"的分支**（Product-Spec 第 71 行）。
   连接形态收敛到单一 `connectionStore`，业务代码读取其中的
   `{kind, remotePlatform, pathSep, label}`，而非各自判断。
   反例参照：`FolderBrowserModal.tsx:85` 靠"路径里有没有反斜杠"猜分隔符——
   这类散落各处的猜测正是迁移时翻译不动、只能重写的死账。
2. **平台语义来自后端，不靠猜**。远端是 Linux 还是 Windows 由后端上报，
   前端据此决定分隔符与大小写敏感性。
3. **默认不监听**。未开启远程时行为与现状完全一致，不引入任何新的监听面。
