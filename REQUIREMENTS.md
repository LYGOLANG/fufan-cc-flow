...
| # | 问题 | 建议默认 |
|---|------|---------|
| 1 | Guest 触发的工具调用，HIL 权限卡弹给谁 | **只弹给 Host**。工具在 Host 机器上执行、动的是 Host 的文件，确认权就该在 Host。可选：Host 显式授权某 Guest 也能确认。 |
| 2 | 两端同时发消息如何处理（会话串行，一轮未完不能插第二轮） | **谁先到谁执行，运行中另一方输入进队列**并显示「排队中」。备选：显式发言权交接。 |
| 3 | 通道走哪条 | 第一版走 **SSH 隧道**：复用现有 web 模式（`endpoint.ts` 的 `isWebOrigin()` 分支即为此设计），零新增服务。验证有真实使用后再上自建中继。 |

**与现有 Spec 的冲突**

`Product-Spec.md` 要求「桌面运行时不开放本地 TCP 端口」「运行期间无后端 TCP 监听」，而本功能的前提就是联网监听。

建议解法：把「不监听端口」从**绝对约束**改为**默认状态**——未开启共享时不监听（保持现有安全姿态），开启共享才启动服务，关闭即停。

**优先级**：低于工作流编排引擎（Phase 12-14）。编排引擎已完成内核，差接入即可实用；远程共享是全新模块，且需处理账户、加密、并发控制，不宜三线并行。
