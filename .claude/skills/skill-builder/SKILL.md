...
[创建规范]
    ...
    - CLAUDE.md 编排 Skill 的顺序和触发，改工作流不改 Skill 内容
    Section 分类：
...
[工作流程]
    了解新 Skill 解决什么、何时触发、输入输出 → 按交互模式找参照 Skill → 读模板定 Section → 逐个填，第一性原则最后一条是联网优先 → 在 .claude/skills/[skill-name]/ 建 SKILL.md，有模板建 templates/ → 自检格式和写作规范 → 在 CLAUDE.md 补 [Skill 调用规则]、[可用技能] 和工作流程。
