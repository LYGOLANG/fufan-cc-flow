...
| Phase | 内容 | 状态 |
|---|---|---|
| 1–2 | 项目初始化后端 API、Project Picker | 2026-07-08 完成 |
| 12–14 | 工作流编排引擎（内核 / 接真实执行 / 编辑器字段） | v0.1.23 发布 |
| 15 | 远程连接底座（SSH 隧道 + 远程 sidecar） | v0.1.24 发布，**8 项验收过 5 项** |
| 16 | 跨机语义修正 | v0.1.25 发布，6 项全过 |

**未启动**

| Phase | 内容 | 说明 |
|---|---|---|
| 3–10 | 纯 Rust 桌面运行时迁移 | 按 Product-Spec v1.12 规划，**一行未动**，当前仍是 Node sidecar |
| 11 | Rust Web/远程 adapter | 依赖 3–10，且 Phase 15 已用 Node 提前实现了 SSH 隧道形态 |

**当前最大的未验证项**：远程连接下的对话、文件树、终端从未实测过 ——
靶机没装 Claude Code CLI（后端只是它的壳，没有 CLI 必然对不了话）。
Phase 15 验收表里第 6/7 项就是这个。
...
| # | 标准 | 状态 |
|---|------|------|
| 1 | 路径大小写/分隔符按目标平台判定 | ❌ **工具写了但没接上** —— 见下方说明 |
| 2 | 新建项目不在无图形界面的机器上挂 120 秒 | ✅ 后端探测 + 应用内目录浏览兜底 |
| 3 | localhost 预览链接指向远程机器而非本机 | ✅ 按需 SSH 端口转发（复用 + 幂等） |
| 4 | 拖拽本机路径不再静默失败 | ✅ 明确拒绝并指向附件上传 |
| 5 | 最近项目列表不混不同机器的路径 | ✅ 按目标平台的绝对路径规则过滤 |
| 6 | 本机连接行为与改造前一致 | ✅ 242 测试全过 / typecheck / lint 0 error |
| 7 | 隧道中断时前端有明确提示而非无限重连 | ❌ 未做 |

**第 1 项曾被错误标为 ✅**（2026-08-01 独立审查纠正）。`hostPath.ts` 的 7 个导出里
**只有 `isAbsolute` 被真正调用过一次**，`pathKey` / `samePath` / `splitPath` /
`basename` / `joinPath` / `relativePath` 全是死代码，仅单测在调。Phase 16 交付内容
点名要改的三处**一处未改**：

| 点名位置 | 现状 |
|---|---|
| `uiStore.ts:5` 的 `toLowerCase()` | 未改 |
| `FileTree.tsx:96` 小写前缀比较 | 未改 |
| `FolderBrowserModal.tsx:107` 靠反斜杠猜分隔符 | 未改 |

那 10 个用例只证明工具函数自身正确，**证明不了任何调用点用上了它**。
这是「没有证据的通过」的典型：写了工具、加了测试、勾了验收，
而真正会出错的代码一行没动。

具体后果（Linux 远端）：项目根 `/home/dev/Proj`，点开 `/home/dev/proj/x.ts`
（Linux 上是另一个真实目录）→ `FileTree.tsx:96` 判为在根之下 → 插入 `@x.ts`
→ 模型去读 `/home/dev/Proj/x.ts`，不存在。

第 7 项独立于其余：`http-chat.ts:48` 的重连状态机没有"地址失效"概念，
断线后会永远以同样的间隔重试同一个地址。远程形态下隧道断了属于常态
（换网、休眠、服务器重启），需要区分"暂时不在"与"这条隧道已经废了"。

**未覆盖**：跨机语义的修复目前只有单测与逻辑推演，尚未在真实远程连接下
逐条走查——那要等靶机装好 Claude Code CLI（见 Phase 15 第 6 项）。
