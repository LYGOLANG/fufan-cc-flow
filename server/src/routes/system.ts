...
const FALLBACK_MODELS: ModelInfo[] = [
  ...
  { id: "sonnet", display_name: "Claude Sonnet 4.6" },
  { id: "haiku", display_name: "Claude Haiku 4.5" },
];
...
// GET /api/system/usage — subscription rate-limit usage (5h + weekly).
// Only available for OAuth/订阅 logins (needs the Claude.ai token).
...
let usageCache: { data: Record<string, unknown>; at: number } | null = null;
...
router.get("/usage", async (_req, res) => {
  try {
    ...
    const payload = { available: true, ...usage };
    usageCache = { data: payload, at: Date.now() };
    res.json(payload);
  } catch (err) {
    logger.warn("[usage] fetch failed: " + String(err));
    // Serve last known good value (if any) so the UI doesn't flicker on a
    // transient 429/network blip.
    if (usageCache) return res.json({ ...usageCache.data, stale: true });
