...
/**
 * F1.12 MCP 配置版本号:GUI 每次增删改 MCP 服务器自增。
 * chatHandler 把它并入常驻进程指纹,下一条消息检测到变化即经 resume 换进程,
 * 新 MCP 配置立即生效(此前改配置要手动重启会话进程)。
 */
let mcpConfigVersion = 0;
export function getMcpConfigVersion(): number {
  return mcpConfigVersion;
}
export function bumpMcpConfigVersion(): void {
  mcpConfigVersion++;
}
...
export class McpService {
  ...
  /**
   * 读取 Codex 自己的 MCP server(只读)—— 来自 ~/.codex/config.toml 的 [mcp_servers.*]。
   * 切到 codex 引擎时用它替代 Claude 的 ~/.claude.json,避免面板显示错对象。
   * codex 的 MCP 由 codex CLI 自己管理,本应用不提供增删改(1A 只读方案)。
   */
  async listCodexServers(): Promise<McpServer[]> {
    const cfgPath = join(homedir(), ".codex", "config.toml");
    try {
      const raw = await fs.readFile(cfgPath, "utf-8");
      return parseCodexMcpServers(raw).map((s) => ({
        name: s.name,
        transport: s.type || (s.url ? "http" : s.command ? "stdio" : "unknown"),
        url: s.url,
        command: s.command,
        args: s.args,
        scope: "user", // config.toml 是用户级
        status: "unknown",
      }));
    } catch (err) {
      logger.warn(`读取 codex config.toml 失败: ${String(err)}`);
      return [];
    }
  }
