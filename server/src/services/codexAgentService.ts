 ...
 *   - Windows 上 `codex` 在 PATH 里解析到的是 `codex.cmd`（npm 全局 shim），
 *     spawnCodex() 因此会用 `cmd /c codex.cmd ...` 包一层 —— proc.kill() 只会
 *     杀掉外层 cmd.exe，真正的 codex 进程会成为孤儿继续跑（已通过日志实测：
 *     abort 后仍收到一条迟到的 session_init）。abort() 必须用 `taskkill /T /F`
 *     杀掉整棵进程树。
 */
