...
## 11. 与「工作流编排引擎」的冲突及决策（2026-07-27）

用户提出为「工作流」实现真正的编排能力（详见 `REQUIREMENTS.md` F5）。该需求与本文冲突，此处记录冲突点与已确认的决策，避免后续实施时反复。

### 11.1 冲突点

| 本文原有约定 | 新需求的要求 |
|---|---|
| 第 5 节 P0-6：前端切到 IPC 后**删除 Node sidecar**、`server-dist`、Node runtime | 编排引擎是重逻辑（状态机、数据传递、失败策略），第一版写在 Node 侧 |
| 第 6 节：**不把旧 Node 和新 Rust 两套后端长期并存** | 双轨期内 Node 侧会新增一个此前不存在的模块 |
| 第 5 节 P0-5：Workflows 只做「现有 CRUD 与文件格式」 | Workflows 从 CRUD 升级为带执行引擎的功能模块 |
| 第 8 节验收：Workflows「行为和数据格式与迁移前一致」 | 数据结构需扩展步骤级字段（输出变量、失败策略） |

### 11.2 决策

**第一版在 Node 侧实现，但核心逻辑与传输层解耦。**

理由：

- Rust 侧对话通道当前仍由 `VITE_RUST_CHAT` 开关关闭、未完成（DEV-PLAN Phase 3）。编排依赖会话管理、权限回路、中断、恢复、后台任务事件——这些能力目前只有 Node 侧的 `claudeAgentService` 完整跑通。等 Rust 通道完成再做，等待期不可控。
- 编排引擎的核心（执行状态机、变量传递、失败处置）是**纯业务逻辑，不依赖传输层**。约束实现方式：通过一个「执行单步」的抽象接口调用底层会话能力，Node 与 Rust 各自提供实现。
- 如此，纯 Rust 迁移时该模块是**翻译**而非重新设计，不构成第 6 节所禁止的「两套后端长期并存」——它只是双轨期内多一个待迁移模块。

### 11.3 对本文的修订

- 第 5 节 P0-5 中 Workflows 一项，迁移目标从「现有 CRUD」扩展为「CRUD + 编排引擎」。
- 第 8 节验收中 Workflows 的「行为与迁移前一致」，以**编排引擎上线后的行为**为基准，而非本文撰写时（2026-07-16）的行为。
- 新增迁移约束：编排引擎的业务逻辑层不得直接依赖 Express/WebSocket 类型，必须经抽象接口访问会话能力，以保证可移植。

### 11.4 未决

- 并行分支、条件分支、循环不在第一版范围。若将来要做，需重新评估是否等到 Rust 侧再实现，避免在待删除运行时上叠加 DAG 执行器。
